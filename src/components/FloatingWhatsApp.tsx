  ...
  return (
    <>
      ...
      {isPopupOpen && (
        <div
          ref={popupRef}
          className="fixed bottom-[86px] right-6 z-60 bg-[#FAF8F5]/95 backdrop-blur-xl border border-[#D5CFBF]/90 shadow-[0_16px_40px_rgba(0,0,0,0.18)] rounded-2xl p-4 w-[240px] animate-in fade-in zoom-in-95 duration-200"
          role="dialog"
      ...
      {/* MAIN BOTTOM-RIGHT FLOATING TRIGGER BUTTON */}
      <div className="fixed bottom-6 right-6 z-60">
        <button
          id="main-support-trigger"
          type="button"
          onClick={handleToggleTrigger}
          className={`w-[56px] h-[56px] sm:w-[58px] sm:h-[58px] rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 shadow-[0_8px_25px_rgba(0,0,0,0.22)] hover:-translate-y-1 hover:shadow-[0_12px_30px_rgba(0,0,0,0.30)] active:translate-y-0 active:scale-95 border border-white/20 backdrop-blur-md ${
            isPopupOpen || isChatbotOpen
              ? 'bg-[#1E2519] text-white rotate-90'
              : 'bg-[#1E2519] text-[#A7F3D0] hover:bg-[#2C3725]'
          }`}
